...
const WMHistoryComponent = ({wmHistory, setEndDate, setStartDate}) => {
    ...
    const filteredWMHistory = wmHistory.filter(
            (event) =>
            event.name.toLowerCase().includes(searchTerm.toLowerCase())

        );
    const dateFilteredActions = filteredWMHistory.filter(() => true);
  ...
  const today = new Date().toISOString().split("T")[0];
    ...
    return(
        <Card style={{ height: "auto", width: "85%", margin: "auto", marginTop:"15px" }}>
        <CardContent
          style={{
            display: "flex",
...
export default WMHistoryComponent;